...
func main() {
	certFile := getenv("TLS_CLIENT_CERT", "../client2.crt")
	keyFile := getenv("TLS_CLIENT_KEY", "../client2.key")
	...
	if err != nil {
		log.Fatalf("load client2 cert: %v", err)
	}
	...
	for i := 0; i < retries; i++ {
		...
		log.Printf("[client2] retrying... (%d/%d) err=%v", i+1, retries, err)
		time.Sleep(time.Duration(sleepSecs) * time.Second)
	}
	if err != nil {
		log.Fatalf("[client2] request failed: %v", err)
	}
	defer resp.Body.Close()
	body, _ := io.ReadAll(resp.Body)
	fmt.Printf("[client2] status=%s\n%s", resp.Status, string(body))
}
